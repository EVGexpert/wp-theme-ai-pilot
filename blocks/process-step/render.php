...
?>
<div <?php echo get_block_wrapper_attributes(['class' => 'aipilot-process-step']); ?> style="display:flex;align-items:flex-start;gap:var(--wp--preset--spacing--md);">
	<div class="aipilot-process-step__icon" style="flex-shrink:0;width:44px;height:44px;border-radius:50%;background:var(--wp--preset--color--dark);display:flex;align-items:center;justify-content:center;">
		<?php if (!$hideN): ?><span style="color:var(--wp--preset--color--surface);font-weight:700;font-size:0.8125rem;"><?php echo (int)$num; ?></span><?php else: echo $svg; endif; ?>
	</div>
	<div class="aipilot-process-step__content" style="flex:1;">
		<h4 class="aipilot-process-step__title" style="margin:0 0 0.25rem;font-size:1rem;font-weight:600;"><?php echo esc_html($title); ?></h4>
		<?php if ($desc): ?><p class="aipilot-process-step__desc" style="margin:0;font-size:0.875rem;color:var(--wp--preset--color--text-secondary);"><?php echo esc_html($desc); ?></p><?php endif; ?>
	</div>
</div>
